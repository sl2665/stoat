...
gtfToGenePred $GTF $ODIR/_tmp/pred
genePredToBed $ODIR/_tmp/pred $ODIR/_tmp/bed
# Match transcript ids to gene names
awk -F"\t" '{n=split($9,a," "); gid=""; tid=""; name="";
	for(i=0;i<=n/2;++i) switch(a[i*2+1]) {
		case "gene_id": gid=a[i*2+2]; break;
		case "transcript_id": tid=a[i*2+2]; break;
		case "gene_name": name=a[i*2+2]; break;
		default: break;}
