...
class hmm5
{
    ...
    hmm5(int Nbases,int refdensity, int maxdensity);
...
hmm5::hmm5(int Nbases,int refdensity=20, int maxdensity=40)
{
    n=Nbases;
    Dref=refdensity;
	Dmax=maxdensity;
...
void hmm5::init(float *v)
{
    ...
    initial[0]=0.1;
    initial[1]=0.9;
    transition[0][1]=0.01;   // transition 
...
int hmm5::iteration(char bon='f',int rdl=200)
{
...
double hmm5::tpos(int s)
{
	return 1.0/go(s-1,s);
};

void hmm5::viterbi(int *path)
{
	double *vp[NSTATE];
